...
bool TestFrame(const uint16_t* frame, bool keyframe)
{
    ...
    cout << "Zdepth Speed: Compresed in " << (t1 - t0) / 1000.f << " msec. Decompresed in " << (t2 - t1) / 1000.f << " msec" << endl;
    ...
    cout << "Quantization+RVL Compression: " << original_bytes << " bytes -> " << compressed.size() << 
        " bytes (ratio = " << original_bytes / (float)compressed.size() << ":1) ("
        << (compressed.size() * 30 * 8) / 1000000.f << " Mbps @ 30 FPS)" << endl;
    cout << "Quantization+RVL Speed: Compresed in " << (t4 - t3) / 1000.f << " msec. Decompresed in " << (t8 - t7) / 1000.f << " msec" << endl;
    ...
    cout << "Quantization+RVL+Zstd Speed: Compresed in " << (t6 - t5 + t4 - t3) / 1000.f << " msec. Decompresed in " << (t8 - t6) / 1000.f << " msec" << endl;
